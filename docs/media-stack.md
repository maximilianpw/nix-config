...
LAN playback stays at `http://kim:32400/web`. On the physical interface, the
host firewall admits Plex TCP 32400 from any IPv4 source, while GDM UDP
32410/32412-32414 remains restricted to `192.168.1.0/24`. It deliberately does
not admit these ports over IPv6 because Kim has globally routable IPv6 addresses,
which would create a direct WAN endpoint without any router port-forward. Update
`physicalLanIpv4Cidr` in `homelab/media/host-services.nix` if the LAN prefix
changes. Confirm Remote Access remains off and that the router has no Plex
port-forward before using the public hostname.
